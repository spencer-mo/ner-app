...
ent = ET.Element('assure:document', {
                 'xmlns:assure': "http://assure.com/assure"})

for left, _id, ent_type, ent_val, right in output:
    ent_wrapper = ET.SubElement(ent, 'assure:entity')
    ent_body = ET.SubElement(ent_wrapper, 'entity:' +
                             ent_type.lower().replace('_', '-'))
